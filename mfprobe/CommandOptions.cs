    ...
    public class CommandOptions : Colipex
    {
        ...
        private bool CheckAndDisplayHelpOrVersion()
        {
            ...
            if (this.IsOption("v"))
            {
                ...
                var copyrightAttributes = thisAssem.GetCustomAttributes(typeof(System.Reflection.AssemblyCopyrightAttribute), false);
            ...
            if (this.IsOption(OptionConstants.OptionHelp))
            {
                ...
                {
                    Help help = new Help(MfCommon.HelpCommandType.MfProbe);
            ...
            out string filesCharacterSet)
        {
            ...
            if (this.IsOption(OptionConstants.OptionSearchWordsCharacterSet))
            {
                searchWordsCharacterSet = this.Options[OptionConstants.OptionSearchWordsCharacterSet].TrimEnd(new char[] { '\x0a', '\x0d' });
                if (searchWordsCharacterSet == Colipex.NonValue)
                {
                    throw new RmsmfException(string.Format(ValidationMessages.MissingEncodingName, "sc"));
                }
            }
            else
            {
            ...
            if (this.IsOption(OptionConstants.OptionDetectionMode))
            {
                ...
                {
                    _encodingDetectionMode = MfCommon.EncodingDetectionType.Normal;
                }
                else
                {
                    if(encJMode == "1")
                    {
                        _encodingDetectionMode = MfCommon.EncodingDetectionType.FirstParty;
                    }
                    else if(encJMode == "3")
                    {
                        _encodingDetectionMode = MfCommon.EncodingDetectionType.ThirdParty;
                    }
                    else
                    {
                        _encodingDetectionMode = MfCommon.EncodingDetectionType.Normal;
        ...
        {
            if (this.IsOption(OptionConstants.OptionProbeMode))
            {
                return true;
            }
            else
            {
                ...
                return this.IsOption(OptionConstants.OptionSearchWords) == false;
            }
        }
        ...
        {
            if (this.IsOption(OptionConstants.OptionOutputFileNamelist))
            {
                if (this.Options[OptionConstants.OptionOutputFileNamelist] == Colipex.NonValue)
                {
                    return "output_filelist.txt";
                }
                else
                {
                    return this.Options[OptionConstants.OptionOutputFileNamelist].TrimEnd(new char[] { '\x0a', '\x0d' });
                }
            }
            ...
            string filesCharacterSet)
        {
            try
            {
                ...
                this.ReplaceEncoding = ResolveEncoding(searchWordsCharacterSet, OptionConstants.CharacterSetDetection);
        ...
        {
            // 検索単語リストファイルオプションの設定
            if (this.IsOption(OptionConstants.OptionSearchWords))
            {
                if (this.Options[OptionConstants.OptionSearchWords] == Colipex.NonValue)
                {
                    throw new RmsmfException(string.Format(ValidationMessages.MissingOptionFileName, "s"));
                }

                this._searchWordsFileName = this.Options[OptionConstants.OptionSearchWords].TrimEnd(new char[] { '\x0a', '\x0d' });

                if (!File.Exists(this._searchWordsFileName))
                {
                    throw new RmsmfException(string.Format(ValidationMessages.FileNotFound, this._searchWordsFileName));
                }
            }
        ...
        private void ValidateRequiredParametersProvided()
        {
            OptionValidator.ValidateAtLeastOneCondition(
                this.IsOption(OptionConstants.OptionSearchWords),
        ...
        private void ValidateSearchWordsHasTargetFiles()
        {
            if (this.IsOption(OptionConstants.OptionSearchWords) && 
                ...
                this.Parameters.Count == 0)
            {
                throw new RmsmfException(ValidationMessages.SearchWordsRequiresTargetFiles);
            }
        }
        ...
        private void ValidateEncodingOptionsDependencies()
        {
            ...
            OptionValidator.ValidateEncodingOptionDependency(
                this.IsOption(OptionConstants.OptionSearchWords),
                this.IsOption(OptionConstants.OptionSearchWordsCharacterSet),
                ValidationMessages.SearchWordsEncodingWithoutSearchWords
            );
            ...
            {
                if (!this.IsOption(OptionConstants.OptionFileNameList) && 
                    !this.IsOption(OptionConstants.OptionOutputFileNamelist))
                {
                    throw new RmsmfException(ValidationMessages.FileListEncodingWithoutFileList);
                }
            }
        }
        ...
        public bool ReadSearchWords()
        {
            ...
            if (lines.Count == 0)
            {
                throw new RmsmfException(string.Format(ValidationMessages.EmptySearchWords, this._searchWordsFileName));
            }
        ...
        /// 文字エンコーディングの自動判定モード
        /// </summary>
        private MfCommon.EncodingDetectionType _encodingDetectionMode = MfCommon.EncodingDetectionType.Normal;
        /// <summary>
        /// 文字エンコーディングの自動判定モード
        /// </summary>
        public MfCommon.EncodingDetectionType EncodingDetectionMode
        {
